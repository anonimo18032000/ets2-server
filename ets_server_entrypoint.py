...
def server_files_exist() -> bool:
    """
    Verifica se o executável principal do servidor está presente.
    Usa a variável env `EXECUTABLE` como caminho para verificar.
    """
...
def generate_moderator_list(mod_list: list) -> str:
    """
    Gera a string exigida pelo arquivo server_config.sii.
    Exemplo:
    ```python
...
def generate_config() -> str:
    ...
    """
    print("[INFO]: Gravando arquivo de configuração...")
    ...
    max_vehicles_total = os.getenv("ETS_SERVER_MAX_VEHICLES_TOTAL", "80")
    max_ai_vehicles_player = os.getenv("ETS_SERVER_MAX_AI_VEHICLES_PLAYER", "40")
    max_ai_vehicles_player_spawn = os.getenv("ETS_SERVER_MAX_AI_VEHICLES_PLAYER_SPAWN", "40")
...
# Este é um AF feio, mas se você indá-lo de forma diferente, ele quebra a configuração...
    ...
    if is_truthy(os.getenv("ETS_SERVER_WRITE_CONFIG", "true")):
        ...
        with open(config_path, "w") as f:
            if f.writable():
                f.write(config)
                f.flush()
                print("[INFO]: Arquivo de configuração escrito.")
            else:
                print(f"[ERROR]: Não foi possível gravar o arquivo de configuração ({config_path}). Verifique as permissões do arquivo!")
    
    if is_truthy(os.getenv("ETS_SERVER_UPDATE_ON_START", "true")) or not server_files_exist():
        print("[INFO]: Atualizando servidor ETS...")
        APP_ID = os.getenv("APP_ID")
        os.system(f"steamcmd +force_install_dir /app +login anonymous +app_update {APP_ID} +quit")
        print("[INFO]: Atualização concluída.")
    else:
        print("[INFO]: Ignorando a atualização do servidor. Para atualizar o conjunto 'ETS_SERVER_UPDATE_ON_START=true'.")
